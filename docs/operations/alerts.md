...
- For `manual_pause_requested`: confirm the pause reason, inspect `/status` and `/orders`, then leave the system halted until the operator explicitly requests `/start`.
